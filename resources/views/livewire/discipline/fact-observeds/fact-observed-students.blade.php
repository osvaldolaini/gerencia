            ...
            <div class="rounded-md col-span-full ">
                <div class="grid grid-cols-2 gap-2 mb-1 sm:grid-cols-4 sm:gap-3 sm:mb-5">
                    @foreach ($selectedStudents as $key => $value)
                        ...
                        {{-- <div class="w-full col-span-2 shadow-xl sm:col-span-3 card card-compact bg-base-100">

                            <div class="card-body dark:text-gray-800">
                                <h2 class="text-sm card-title"> Al. {{ $value['nick'] }}</h2>
                                <p class="py-0 my-0 text-sm">T. {{ $value['class'] }}</p>
                                <div class="justify-end card-actions">
                                    <button wire:click="removeStudents({{ $key }})"
                                        class="btn {{ $value['sex'] == 'F' ? 'btn-secondary' : 'btn-info' }}">Excluir
                                        <svg xmlns="http://www.w3.org/2000/svg" fill="none" viewBox="0 0 24 24"
                                            class="inline-block w-4 h-4 cursor-pointer stroke-current">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                                                d="M6 18L18 6M6 6l12 12"></path>
