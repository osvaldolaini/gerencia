<div>
    ...
    <div class='flex justify-center col-span-2 text-right'>
        <div class="p-0 tooltip tooltip-top" data-tip="imprimir batalhão">
            <a class="btn btn-outline btn-success" target="_blank" wire:click='printBattalion()'>
                imprimir <svg xmlns="http://www.w3.org/2000/svg" class="w-6 h-6" fill="currentColor" viewBox="0 0 512 512"
                    xml:space="preserve">
                    <g>
    ...
    @if ($school_battalion->where('active', 1)->sortBy('order')->groupBy('posto_grad')->count() > 0)
        <small
    ...
    <div
        class="grid grid-cols-{{ $school_battalion->where('active', 1)->sortBy('order')->groupBy('posto_grad')->count() }} gap-4">
