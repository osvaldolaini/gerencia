        ...
        <x-slot name="head">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr scope="col" class="text-gray-500 dark:text-gray-400">
                    <th scope="col" wire:click="addSort('year')"
                        class="px-4 py-1 text-sm font-normal text-left text-gray-500 dark:text-gray-400">
                        <x-layout.table-sort-button :sorts='$sorts' field="year">
                            Ano
                ...
                @foreach ($dataTable as $item)
                    <tr>
                        ...
                        <td class="items-center px-4 py-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                            <a href="{{ route('school-classes-year-list', $item->id) }}"
    ...
    <x-dialog-modal wire:model="showModalForm" maxWidth="4xl">
        ...
        <x-slot name="content">
            @if ($school_classes_years)
                @livewire('settings.school-classes-years.school-classes-year-form', ['school_classes_years' => $school_classes_years], key($school_classes_years->id))
            @else
                @livewire('settings.school-classes-years.school-classes-year-form')
            @endif
        </x-slot>
        <x-slot name="footer">
