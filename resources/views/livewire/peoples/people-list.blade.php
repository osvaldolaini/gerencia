<div x-init>
        ...
        <x-slot name="head">
            <thead class="bg-gray-50 dark:bg-gray-800">
                <tr scope="col" class="text-gray-500 dark:text-gray-400">
                    ...
                    <th scope="col" wire:click="addSort('function')"
                        class="px-4 py-1 text-sm font-normal text-center text-gray-500 dark:text-gray-400">
                        <x-layout.table-sort-button :sorts='$sorts' field="function">
                            Nome de guerra
                        </x-layout.table-sort-button>
                    </th>
                    <th scope="col" wire:click="addSort('function')"
    ...
    <x-dialog-modal wire:model="showModalForm" maxWidth="4xl">
        ...
        <x-slot name="content">
            @if ($peoples)
                @livewire('peoples.people-form', ['peoples' => $peoples], key($peoples->id))
            @else
                @livewire('peoples.people-form')
            @endif
        </x-slot>
        <x-slot name="footer">
