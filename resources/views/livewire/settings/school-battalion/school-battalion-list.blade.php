<div>
                ...
                @foreach ($dataTable as $item)
                    <tr>
                        ...
                        <td class="items-center px-4 py-1 text-sm font-normal text-gray-500 dark:text-gray-400">
                            <a href="{{ route('school-battalion-students-grade', $item->id) }}"
    ...
    <x-dialog-modal wire:model="showModalForm" maxWidth="4xl">
        ...
        <x-slot name="content">
            @if ($school_battalion)
                @livewire('settings.school-battalion.school-battalion-form', ['school_battalion' => $school_battalion], key($school_battalion->id))
            @else
                @livewire('settings.school-battalion.school-battalion-form')
            @endif
        </x-slot>
        <x-slot name="footer">
