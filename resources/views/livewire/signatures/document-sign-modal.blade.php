<div class="mt-2">
    ...
    @if ($currentDocument?->file_path != null)
        <div class="p-0 tooltip tooltip-top" data-tip="Imprimir assinado">
            <a href="{{ route('documents.show', $currentDocument->uuid) }}" target="_blank"
                class="btn btn-outline dark:btn-info">
                <x-layout.svg.pdf class="w-6 h-6 "></x-layout.svg.pdf>
                Verificação
            </a>
        </div>
        <div class="p-0 tooltip tooltip-top mt-2" data-tip="Baixar assinado">
            <button wire:click="download('{{ $currentDocument->uuid }}')" class="flex btn btn-outline dark:btn-info">
        ...
        <div class="p-0 tooltip tooltip-top mt-2" data-tip="Revogar">
            {{-- <button class="btn  btn-error" wire:click="revokeDocument"
                wire:confirm="Tem certeza que deseja revogar este documento oficial?"> --}}
            <button class="btn  btn-error" wire:click="showModalDelete">
    ...
    @else
        <!-- BOTÃO VISÍVEL -->
        <div class="p-0 tooltip tooltip-top" data-tip="Assinar">
            <button wire:click="openModal" class="btn btn-outline dark:btn-info ">
                <svg fill="currentColor" class="w-6 h-6 " viewBox="0 0 14 14" role="img" focusable="false"
        ...
        <div class="fixed inset-0 z-50 flex items-center justify-center bg-black/50">
            <div class="w-full max-w-md p-6 bg-white rounded-lg shadow-lg">
                ...
                <div class="mb-4 text-sm text-gray-600">
                    ...
                    <div class="mb-6">
                        <div class="mb-3">
                            ...
                            @if (!$currentDocument)
                                <div class="alert alert-info mt-2">
                                    <span>
                                        Documento ainda não possui assinaturas.
                                    </span>
                                </div>
                            @elseif ($currentDocument->status->isCurrent() && $nextRole)
                                <div class="alert alert-warning mt-2">
                                    <span>
                                        Documento em processo de assinatura.
                                    </span>
                                </div>
                            @elseif ($currentDocument->status->isCompleted())
                                <div class="alert alert-success mt-2">
                                    <span>
                                        Documento totalmente assinado.
                                    </span>
                                </div>
                            @elseif ($currentDocument->status->isCancelled())
                        ...
                        <div class="rounded-lg border border-base-300 p-3">
                            ...
                            @foreach ($requiredRoles as $role)
                                @php
                                    $signed = in_array($role, $signedRoles, true);
                                @endphp
                                <div class="flex items-center justify-between py-2 border-b last:border-0">
                                    <div class="flex items-center gap-2">
                                        @if ($signed)
                                            <span class="text-success text-lg">✔</span>
                                        @elseif($nextRole === $role)
                    ...
                    {{-- @if ($currentDocument)
                        <div>
                            <strong>Documento já possui assinatura</strong>
                        </div>
                    @else
                        <div>
                    ...
                    <div class="mt-2">
                        Próxima assinatura:
                        <strong>{{ $nextRole?->label() ?? 'Nenhuma definida' }}</strong>
                    </div> --}}
                ...
                <div class="mb-4 text-gray-800">
                    ...
                    <select wire:model="selectedSigner" class="w-full border rounded p-2">
                ...
                <div class="my-4">
                    <label class="label">
                        <span class="label-text">Senha da assinatura</span>
                    </label>

                    <input type="password" wire:model.defer="password"
                        class="w-full border rounded p-2 @error('password') input-error @enderror">
                ...
                <button wire:click="sign" class="w-full py-2 text-white bg-green-600 rounded">
                    Assinar
                </button>
    ...
    <x-confirmation-modal wire:model="showJetModal">
        ...
        <x-slot name="content">
            <h2 class="h2">Tem certeza que deseja revogar este documento oficial?</h2>
        ...
        <x-slot name="footer">
            ...
            <x-danger-button class="ml-2" wire:click="revokeDocument()" wire:loading.attr="disabled">
                Revogar
            </x-danger-button>
        </x-slot>
